...
/*
  ...
  then
    ...
    case "$osname" in
      Dar )
        env GOOS=darwin GOARCH=amd64 go build -o xtract.Darwin xtract.go common.go
        env GOOS=darwin GOARCH=amd64 go build -o rchive.Darwin rchive.go common.go
      ;;
      Lin )
        env GOOS=linux GOARCH=amd64 go build -o xtract.Linux xtract.go common.go
        env GOOS=linux GOARCH=amd64 go build -o rchive.Linux rchive.go common.go
      ;;
      CYG | MIN )
        env GOOS=windows GOARCH=386 go build -o xtract.CYGWIN_NT xtract.go common.go
        env GOOS=windows GOARCH=386 go build -o rchive.CYGWIN_NT rchive.go common.go
      ;;
    esac
  fi
*/
...
func RepairEncodedMarkup(str string) string {
	...
	for i, ch := range str {
		...
		if ch == '<' {
			...
			if ok {
				adv := len(sub) - 1
				// do not convert if flanked by spaces - it may be a scientific symbol, e.g., fragments <i> in PMID 9698410
...
func PrepareQuery(str string) string {
	...
	// break terms at punctuation, and at non-ASCII characters, allowing Boolean control symbols,
	// allowing asterisk to indicate truncation wildcard, and allowing underscore for protected terms
...
func StreamXML(in *XMLReader, tokens func(Token)) {
	...
	parseLevel = func(strt, attr, prnt string) (*Node, bool) {
		...
		for {
			tag, ctype, name, attr, idx, _ := nextToken(Idx)
			if tag == ISCLOSED {
				break
			}
			Idx = idx
